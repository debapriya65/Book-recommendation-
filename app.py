...
connection = mysql.connector.connect(host="localhost", user="root", password="28550093", database="login")
...
def suggestion():
    if 'user_id' in session:
        if "username" in session:
            username= session["username"]
    cursor.execute("""SELECT *FROM `user_rating` WHERE `username` LIKE '{}'""".format(username))
    q_res = cursor.fetchall()
    book_list = [all_books_with_avg_num[all_books_with_avg_num['ISBN'].isin (list([tup[2]]))]['Book-Title'].unique()[0] for tup in q_res]
    rating_list = [tup[3] for tup in q_res]
    # print(book_list)
    # print(rating_list)
    title_list=model.predict(book_list,rating_list)
    ...
    isbn = list(popular_df['ISBN'].values)
    read_isbn_list = [tup[1] for tup in book_read_list]
    return render_template('suggestion.html',book_name=list(all_books_with_avg_num[all_books_with_avg_num['Book-Title'].isin (title_list)]['Book-Title'].unique()),
                               book_author=list(all_books_with_avg_num[all_books_with_avg_num['Book-Title'].isin (title_list)]['Book-Author'].unique()),
                               book_image=list(all_books_with_avg_num[all_books_with_avg_num['Book-Title'].isin (title_list)]['Image-URL-M'].unique()),
                               book_votes=list(all_books_with_avg_num[all_books_with_avg_num['Book-Title'].isin (title_list)]['num_rating'].unique()),
                               book_avg_rating=list(np.around(all_books_with_avg_num[all_books_with_avg_num['Book-Title'].isin (title_list)]['avg_rating'].unique())),
                               isbn=list(all_books_with_avg_num[all_books_with_avg_num['Book-Title'].isin (title_list)]['ISBN'].unique()),
                               username=username,
                               read_isbn_list=read_isbn_list
                               )
## delete all rows
...
def download():
    if request.method == 'POST':
        print( "src/"+request.get_json(force=True)['title']+".pdf")
        return send_from_directory(app.static_folder, "src/"+request.get_json(force=True)['title']+".pdf")

## delete all rows
@app.route('/delete', methods=['post'])
def delete():
    cursor.execute(""" DELETE FROM `user_rating`; """)
...
def all_books():
    if 'user_id' in session:
        ...
        return render_template('all_books.html',
                               book_name=list(all_books_with_avg_num['Book-Title'].values),
                               book_author=list(all_books_with_avg_num['Book-Author'].values),
                               book_image=list(all_books_with_avg_num['Image-URL-M'].values),
                               book_votes=list(all_books_with_avg_num['num_rating'].values),
                               book_avg_rating=list(np.around(all_books_with_avg_num['avg_rating'].values, 2)),
                               username=username,
                               read_isbn_list=read_isbn_list,
                               isbn=isbn
                               )
...
def searched_books():
    if 'user_id' in session:
        ...
        close_match_book_list = difflib.get_close_matches(search_user_input_book, list_of_all_book_names,10,cutoff=0.01)  # kaorn convert kora lage difflib calaite gele frame er upor hoy na
